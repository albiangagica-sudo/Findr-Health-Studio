...
// Sub-components moved outside to prevent re-definitions on every render
...
const NavLink = ({ to, children, icon: Icon, mobile = false, active, onClick }: { to: string, children: React.ReactNode, icon?: any, mobile?: boolean, active?: boolean, onClick?: () => void }) => {
  const baseClasses = mobile 
    ? "text-base font-bold py-4 transition-colors uppercase tracking-tight flex items-center gap-4 px-5 rounded-2xl hover:bg-gray-50"
    : "text-xs font-black uppercase tracking-widest transition-all relative group flex items-center gap-2 px-4 py-2 rounded-xl hover:bg-white hover:shadow-sm";
  ...
  return (
    ...
    >
      ...
      {!mobile && active && (
        <motion.span 
          layoutId="nav-pill"
          className="absolute inset-0 bg-white -z-10 rounded-xl shadow-sm border border-gray-100"
...
export default function Navbar({ onUploadClick }: { onUploadClick?: () => void }) {
  ...
  const MobileLegalLink = ({ children, icon: Icon, onClick }: { children: React.ReactNode, icon: any, onClick?: () => void }) => (
    <a 
      href="#" 
      onClick={(e) => { e.preventDefault(); onClick?.(); setIsMenuOpen(false); }} 
      className="text-sm font-bold py-3 px-4 text-black/60 hover:text-black transition-colors uppercase tracking-tight flex items-center gap-3 rounded-xl hover:bg-gray-100"
    >
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[2000] px-4 pt-4 md:pt-6">
      <div className="max-w-7xl mx-auto h-16 bg-white/70 backdrop-blur-xl border border-gray-100 rounded-[2rem] shadow-[0_8px_32px_rgba(0,0,0,0.04)] flex items-center justify-between px-5 md:px-6 relative z-[2010]">
        ...
        <div className="hidden md:flex items-center gap-4 lg:gap-8">
          ...
          <button 
             ...
             className="px-6 h-11 bg-zest text-black rounded-xl text-xs font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all shadow-lg shadow-zest/10 active:scale-95 flex items-center justify-center gap-2"
          >
          ...
          {user ? (
            <div className="flex items-center gap-4 border-l border-gray-100 pl-4 ml-2">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-findr-light rounded-full border border-findr/20">
                {user.photoURL ? (
                  ...
                  <div className="w-6 h-6 rounded-full bg-findr flex items-center justify-center text-white">
                    <User size={12} />
                  </div>
                )}
              ...
              <button 
                onClick={logout}
                className="p-2.5 hover:bg-red-50 hover:text-red-500 rounded-full transition-all active:scale-95"
              >
                <LogOut size={18} />
              </button>
            </div>
            ...
            <button 
              onClick={login}
              className="px-8 h-11 bg-black text-white rounded-2xl text-sm font-black hover:bg-cobalt transition-all hover:scale-105 active:scale-95 shadow-lg shadow-black/10 flex items-center justify-center"
            >
              Login / Sign up
            </button>
          )}
        ...
        {/* Mobile menu button - Enlarged tap area and high z-index */}
        <button 
           className="md:hidden w-12 h-12 flex items-center justify-center text-black active:scale-90 transition-transform relative z-[2011] select-none touch-manipulation"
           onClick={() => {
      ...
      {/* Mobile Nav Tray - Inside nav for context sharing */}
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            key="mobile-nav-root"
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[2005] md:hidden"
          >
            {/* Backdrop */}
            <motion.div 
              initial={{ opacity: 0 }}
              ...
              <div className="flex-1 overflow-y-auto px-8 pb-12 pt-4 overscroll-contain">
                <div className="flex flex-col gap-8">
                  ...
                  <button 
                    ...
                    className="w-full py-6 bg-zest text-black rounded-[2rem] font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 shadow-xl shadow-zest/20 active:scale-95 transition-all"
                  >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <a 
                      ...
                      onClick={() => setIsMenuOpen(false)}
                      className="text-lg font-bold py-4 px-5 text-black/60 hover:text-black transition-colors uppercase tracking-tight flex items-center gap-4 rounded-2xl hover:bg-gray-100"
                    >
                  ...
                  <div className="pt-8 border-t border-gray-200">
                    {!user ? (
                      <button 
                        ...
                        className="w-full py-6 bg-black text-white rounded-[2rem] font-black uppercase tracking-widest text-xs shadow-xl active:scale-95 transition-all"
                      >
                        Login / Sign up
                      </button>
                    ) : (
                      <div className="flex flex-col gap-6">
                        <div className="flex items-center gap-5 px-6 py-4 bg-gray-100 rounded-[2.5rem] border border-gray-200">
                          {user.photoURL ? (
                            ...
                            <div className="w-14 h-14 rounded-full bg-findr flex items-center justify-center text-white">
                              <User size={28} />
                            </div>
                          )}
                          <div>
                        ...
                        <div className="flex flex-col gap-2">
                          <button 
                            ...
                            className="w-full py-6 text-red-500 bg-red-100/50 hover:bg-red-100 rounded-[2rem] font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-colors active:scale-95 shadow-sm"
                          >
